                ...
                literal("economy").requires(source -> source.hasPermissionLevel(2))
                        .then(literal("deposit").then(argument("player", EntityArgumentType.player())
                                .then(argument("cents", LongArgumentType.longArg(1)).executes(context -> {
                                    ...
                                    EconomyState.get(context.getSource().getServer()).issueMoney(EconomyState.playerAccount(player.getUuid()), cents,
                                            "Operator-Testemission", context.getSource().getEntity() == null ? "CONSOLE" : context.getSource().getEntity().getUuidAsString());
