...
public class LandSelectionState extends PersistentState {
    ...
    public boolean add(UUID player,String dimension,BlockPos pos){LandSelection old=get(player);if(old==null||!old.dimension().equals(dimension)||old.points().size()>=ConfigManager.claims().maximumPolygonPoints)return false;List<String> points=new ArrayList<>(old.points());String encoded=encode(pos);if(points.isEmpty()||!points.get(points.size()-1).equals(encoded))points.add(encoded);selections.put(player.toString(),new LandSelection(dimension,points));decodedCache.remove(player.toString());markDirty();return true;}
    public void undo(UUID player){LandSelection old=get(player);if(old==null||old.points().isEmpty())return;List<String> points=new ArrayList<>(old.points());points.remove(points.size()-1);selections.put(player.toString(),new LandSelection(old.dimension(),points));decodedCache.remove(player.toString());markDirty();}
