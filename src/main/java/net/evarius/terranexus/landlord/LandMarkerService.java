...
public final class LandMarkerService {
    ...
    private static List<BlockPos> decode(List<String> values,int y){List<BlockPos> points=new ArrayList<>();for(String value:values){String[] pair=value.split(",");if(pair.length==2)try{points.add(new BlockPos(Integer.parseInt(pair[0]),y,Integer.parseInt(pair[1])));}catch(NumberFormatException ignored){}}return points;}
