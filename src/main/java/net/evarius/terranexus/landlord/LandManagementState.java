...
public class LandManagementState extends PersistentState {
    ...
    public AdministrativeArea createArea(String name,String type,String parent,String ownerType,String ownerId){String id=UUID.randomUUID().toString();AdministrativeArea area=new AdministrativeArea(id,name,type,parent,ownerType,ownerId);areas.put(id,area);areaCache=null;markDirty();return area;}
    public List<AdministrativeArea> areas(){if(areaCache==null)areaCache=areas.values().stream().sorted(Comparator.comparing(AdministrativeArea::type).thenComparing(AdministrativeArea::name)).toList();return areaCache;}
    ...
    public static long periodMillis(int days){return (long)Math.max(1,days)*ConfigManager.claims().rentDayDurationMinutes*60_000L;}
    public void processRents(MinecraftServer server,long now){EconomyState economy=EconomyState.get(server);boolean changed=false;for(LandLease lease:new ArrayList<>(leases.values())){if(!lease.active()||lease.nextDueAt()>now)continue;UUID tenantId;try{tenantId=UUID.fromString(lease.tenantId());}catch(IllegalArgumentException ignored){leases.remove(lease.propertyId());changed=true;continue;}String institution=lease.landlordAccount().startsWith("institution:")?lease.landlordAccount().substring("institution:".length()):"";boolean paid=economy.transfer(EconomyState.playerAccount(tenantId),lease.landlordAccount(),lease.rent(),"Grundstücksmiete · "+lease.propertyId(),"SYSTEM",institution,"RENT");int missed=paid?0:lease.missedPayments()+1;if(missed>=ConfigManager.claims().maximumMissedRentPayments){leases.remove(lease.propertyId());var online=server.getPlayerManager().getPlayer(tenantId);if(online!=null)online.sendMessage(net.minecraft.text.Text.literal("Mietvertrag nach offenen Zahlungen beendet."),false);}else{long next=now+periodMillis(lease.periodDays());leases.put(lease.propertyId(),new LandLease(lease.propertyId(),lease.landlordAccount(),lease.tenantId(),lease.rent(),lease.deposit(),lease.periodDays(),next,missed,true));}changed=true;}if(changed)markDirty();}
}
