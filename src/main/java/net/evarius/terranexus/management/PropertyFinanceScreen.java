...
public final class PropertyFinanceScreen {
    ...
    private static void publicLeaseMoney(ServerPlayerEntity owner,LandProperty property){money(owner,"Miete pro Zeitraum",rent->money(owner,"Kaution",deposit->integer(owner,"Zahlungsintervall in Tagen",days->integerAllowZero(owner,"Vertragszahlungen (0 = unbefristet)",payments->choosePublicRenewal(owner,property,rent,deposit,days,payments)))));}
    ...
    private static void choosePublicRenewal(ServerPlayerEntity owner,LandProperty property,long rent,long deposit,int days,int payments){SimpleInventory inventory=new SimpleInventory(54);Map<Integer,Consumer<net.minecraft.entity.player.PlayerEntity>> actions=new HashMap<>();ManagementHubScreen.display(inventory,4,Items.CLOCK,"Öffentliches Mietangebot",payments==0?"Unbefristete Wohnung":payments+" Zahlung(en) · geeignet für Hotelbuchungen");button(inventory,actions,20,Items.LIME_DYE,"Automatisch verlängern","Buchung erneuert sich nach der Laufzeit",ignored->createPublicLease(owner,property,rent,deposit,days,payments,true));button(inventory,actions,24,Items.RED_DYE,"Festes Ende","Buchung endet nach der Laufzeit",ignored->createPublicLease(owner,property,rent,deposit,days,payments,false));button(inventory,actions,8,Items.ARROW,"Zurück","Vertragsansicht",ignored->open(owner,property));openMenu(owner,inventory,actions,"Self-Check-in konfigurieren");}
    ...
    private static void createPublicLease(ServerPlayerEntity owner,LandProperty property,long rent,long deposit,int days,int payments,boolean renew){showResult(owner,LandTradeService.offerPublicLease(owner,property.id(),rent,deposit,days,payments,renew));openLatest(owner,property.id());}
    ...
    private static void trustedPermissions(ServerPlayerEntity manager,LandProperty property,CitizenIdentity identity){LandManagementState state=LandManagementState.get(manager.getServer());LandAccess access=state.access(property.id());SimpleInventory inventory=new SimpleInventory(54);Map<Integer,Consumer<net.minecraft.entity.player.PlayerEntity>> actions=new HashMap<>();ManagementHubScreen.display(inventory,4,Items.PLAYER_HEAD,identity.firstName()+" "+identity.lastName(),"Individuelle Grundstücksrechte");individualToggle(inventory,actions,19,Items.BRICKS,"Bauen",LandAccess.BUILD,manager,property,identity,access);individualToggle(inventory,actions,21,Items.OAK_DOOR,"Interaktion",LandAccess.INTERACT,manager,property,identity,access);individualToggle(inventory,actions,23,Items.CHEST,"Container öffnen",LandAccess.CONTAINERS,manager,property,identity,access);individualToggle(inventory,actions,25,Items.REDSTONE,"Redstone",LandAccess.REDSTONE,manager,property,identity,access);individualToggle(inventory,actions,27,Items.TRIPWIRE_HOOK,"Objektsperren verwalten",LandAccess.MANAGE_CONTAINERS,manager,property,identity,access);ManagementHubScreen.display(inventory,8,Items.ARROW,"Zurück","Personenauswahl");actions.put(8,x->selectTrusted(manager,property));openMenu(manager,inventory,actions,"Einzelrechte");}
